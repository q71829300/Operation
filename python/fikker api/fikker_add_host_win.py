...
#證書、key放置路徑
mypath = r'C:\Users\Desktop'
#列出目錄下所有檔案
files = listdir(mypath)

#帶參數，cdn可帶多個參數
parser = argparse.ArgumentParser(description='請帶 -i 參數在加上IP，前面參數為新的CDN IP，後面參數為該CDN的源站IP，範例:python -u "py檔案位置" -cdn cdn_ip -src source_ip')
...
def crt_for_domain(crt, key):
    #add host
    fname = r'C:\Users\Desktop' + crt
    with open(fname) as f:
        cert_pem = f.read()
        f.close()  
    fname = r'C:\Users\Desktop' + key
    with open(fname) as f:
        cert_key = f.read()
        f.close()        
    ...
    for i in range(len(domain_list)):
        ...
        url = 'http://fikker_ip/fikker/webcache.fik?'
        # 取得SessionID，要帶帳密
        my_data = 'type=sign&cmd=in&Username=admin&Password=123456'
        all_url= url+my_data
